                ...
                for xy, zp in support:
                    if type(xy) == type(()):  # xp is a pair (x,y) and you need interpolation
                        for plan in zp:  # /!\ only list of plans is allowed for now
                            z[jvar][ipt][it] = 0.
                            ln, bn = xy
                            for inod in range(len(bn)): z[jvar][ipt][it] += bn[inod] * VARSOR[
                                ln[inod] + plan * NPOIN3 / NPLAN]
    ...
    try:
        ...
        for e, i in zip(IKLE, range(len(IKLE))):
            ...
            for k in [0, 1, 2]:
                ibar += 1
                if showbar: pbar.update(ibar)
                if (e[k], e[(k + 1) % 3]) not in nk:
                    bounders.update({(e[(k + 1) % 3], e[k]): i})
                else:
    ...
    for it in range(len(time)):  # it is from 0 to len(time)-1
        ...
        for ivar in range(NVAR):  # ivar is from 0 to NVAR-1
            ...
            if ivar in varsIndexes:  # extraction of a particular variable
                VARSOR = unpack(endian + str(NPOIN3) + ftype, f.read(fsize * NPOIN3))
                for ipt in range(
                        len(support)):  # ipt is from 0 to lens-1 (= all points extracted and all plans extracted)
                    xy, zp = support[ipt]
                    for ipl in range(len(zp)):  # /!\ only list of plans is allowed for now
                        z[varsIndexes.index(ivar)][it][ipl][ipt] = 0.
                        ln, bn = xy
                        for inod in range(len(bn)): z[varsIndexes.index(ivar)][it][ipl][ipt] += bn[inod] * VARSOR[
                            ln[inod] + zp[ipl] * NPOIN3 / NPLAN]
            else:
        ...
        for ivar in range(NVAR):  # ivar is from 0 to NVAR-1
            ...
            if ivar in varsIndexes:  # extraction of a particular variable
                ...
                for ipl in range(len(support)):  # ipt is from 0 to len(support) (= all plans extracted)
                    z[varsIndexes.index(ivar)][it][ipl] = VARSOR[support[ipl] * NPOIN3 / NPLAN:(support[
                                                                                                    ipl] + 1) * NPOIN3 / NPLAN]
            else:
                f.read(fsize * NPOIN3)  # the file pointer advances through all records to keep on track
            f.read(4)
...
class SELAFIN:
    ...
    def getHeaderIntegersSLF(self):
        ...
        f.seek(4, 1)
        self.IKLE3 = np.array(
            unpack(endian + str(self.NELEM3 * self.NDP3) + 'i', f.read(4 * self.NELEM3 * self.NDP3))) - 1
        f.seek(4, 1)
    ...
    def appendCoreTimeSLF(self, t):
        ...
        # Print time record
        if type(t) == type(0.0):
            f.write(pack(endian + 'i' + ftype + 'i', 4, t, 4))
        else:
    ...
    def getVALUES(self, t):
        ...
        for v in self.alterZnames:
            ...
            for iv in range(len(self.CLDNAMES)):
                if v.lower() in self.CLDNAMES[iv].lower(): VARSOR[iv + self.NBV1] = self.alterZm * VARSOR[
                    iv + self.NBV1] + self.alterZp
        return VARSOR
        ...
        for slf in self.slfs[1:]:
            ...
            if len(slf.tags['times']) != len(self.slfs[0].tags['times']):
                ...
                same = same and (accuracy > \
                                 np.max(slf.tags['times'] - self.slfs[0].tags['times']) - np.min(
                                     slf.tags['times'] - self.slfs[0].tags['times']))
        return same
    ...
    def getSERIES(self, nodes, varsIndexes=[]):
        ...
        if len(self.slfs) > 0:
            ...
            for islf, slf in zip(range(len(self.slfs)), self.slfs):
                ...
                subGnodes = np.compress(mproc == islf,
                                        np.array(zip(range(len(nodes)), nodes), dtype=[('r', int), ('n', int)]))
                subLnodes = np.searchsorted(np.sort(slf.IPOB2),
                                            subGnodes['n']) + 1  # /!\ why does this work in a sorted way ?
        ...
        for slf in self.slfs:  # TODO: do this loop in python parallel with a bottle neck at islf.getVALUES(t)
            ...
            if self.slf.NPOIN2 == islf.NPOIN2:
                ...
                if islf.NPLAN > 1:
                    ...
                    sub.IPOB3 = np.ravel(np.add(np.repeat(sub.IPOB2, islf.NPLAN).reshape((sub.NPOIN2, islf.NPLAN)),
                                                sub.NPOIN2 * np.arange(islf.NPLAN)).T)
                    sub.IKLE3 = \
                        np.repeat(sub.NPOIN2 * np.arange(islf.NPLAN - 1), sub.NELEM2 * islf.NDP3).reshape(
                            (sub.NELEM2 * (islf.NPLAN - 1), islf.NDP3)) + \
                        np.tile(np.add(np.tile(sub.IKLE2, 2), np.repeat(sub.NPOIN2 * np.arange(2), sub.NDP2)),
                                (islf.NPLAN - 1, 1))
                else:
            ...
            # ~~> filtered partitions / non reversable !
                ...
                GKLE2 = islf.IKLE2[np.where(
                    np.sum(np.in1d(islf.IKLE2, np.sort(indices)).reshape(islf.NELEM2, islf.NDP2), axis=1) == islf.NDP2)]
                ...
                for k in range(len(GKLE2)):
                    for ki in range(islf.NDP2): sub.IKLE2[k][ki] = KNOGL[
                        GKLE2[k][ki]]  # /!\ sub.IKLE2 has a local numbering, fit to the boundary elements
            ...
            for t in range(len(islf.tags['times'])):
                ...
                for ivar, var in zip(range(sub.NVAR), islf.getVALUES(t)):
                    for n in range(sub.NPLAN): VARSORS[ivar][n * sub.NPOIN2:(n + 1) * sub.NPOIN2] = \
